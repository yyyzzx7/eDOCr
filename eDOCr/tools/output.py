...
def mask_the_drawing(img, infoblock_dict, gdt_dict, dimension_dict, cl_frame, color_palette):
    ...
    for gdt in gdt_dict:
        ...
        flag = pred.get('flag')
        box = np.array(
            [(cls[0].x, cls[0].y), (cls[0].x + cls[0].w, cls[0].y), (cls[0].x + cls[0].w, cls[0].y + cls[0].h),
             (cls[0].x, cls[0].y + cls[0].h)], np.int32)
        mask_img = mask_infobox(box, text, mask_img, color_palette.get('gdts'), color_palette.get('flag'), flag)
        for cl in cls[1:]:
            box = np.array([(cl.x, cl.y), (cl.x + cl.w, cl.y), (cl.x + cl.w, cl.y + cl.h), (cl.x, cl.y + cl.h)],
                           np.int32)
...
def record_data(dest_DIR, filename, infoblock_dict, gdt_dict, dimension_dict, index=0):
    ...
    with open(os.path.join(dest_DIR, filename + '_' + str(index) + '_dimensions.csv'), 'w',
